...
class DisciplineMacroListScreen extends StatefulWidget {
  const DisciplineMacroListScreen({super.key});

  @override
  State<DisciplineMacroListScreen> createState() =>
      _DisciplineMacroListScreenState();
}

class _DisciplineMacroListScreenState extends State<DisciplineMacroListScreen> {
  bool _loading = true;
  ...
  Future<void> _load() async {
    ...
    try {
      _items = await NativeService.getAllDisciplineMacros();
    } catch (_) {}
  ...
  Future<void> _toggle(Map<String, dynamic> item) async {
    ...
    try {
      await NativeService.updateDisciplineMacro({
        ...item,
        'id': id,
  ...
  Future<void> _delete(Map<String, dynamic> item) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Eliminar "${item['name'] ?? 'Disciplina'}"?'),
        actions: [
          TextButton(
    ...
    if (ok != true) return;
    try {
      await NativeService.deleteDisciplineMacro(id);
      await _load();
    } catch (e) {
      if (mounted) context.showSnack('Error: $e', isError: true);
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Macros de Disciplina'),
        actions: [
          IconButton(
            onPressed: _exportDisciplines,
            icon: const Icon(Icons.file_upload_rounded),
            tooltip: 'Exportar',
          ),
          IconButton(
            onPressed: _importDisciplines,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'Crea tu primera macro de disciplina.',
  ...
  Future<void> _exportDisciplines() async {
    try {
      final res = await NativeService.exportDisciplineMacros();
      if (res['success'] != true) {
        if (mounted) context.showSnack('No se pudo exportar', isError: true);
        return;
      }
      final json = const JsonEncoder.withIndent('  ')
          .convert({'disciplines': res['disciplines'] ?? []});
      if (!mounted) return;
      await showDialog<void>(
        context: context,
        builder: (_) => AlertDialog(
          title: const Text('Exportar disciplina'),
          content: SizedBox(
  ...
  Future<void> _importDisciplines() async {
    final controller = TextEditingController();
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Importar disciplina'),
        content: TextField(
    ...
    try {
      ...
      final validation = validateDisciplineImportPayload(
      ...
      await NativeService.importDisciplineMacros(validation.payload);
      await _load();
      if (mounted) context.showSnack('Disciplina importada');
    } catch (e) {
      if (mounted) context.showSnack('Error: $e', isError: true);
    }
  }
